...
cuppa <- 
fread(paste0( CUPPA_DIR, "cuppa_update_processed.csv")) %>%
...
fwrite(summaries, paste0(SHARE_DIR, "cohort_summary.csv"))

neo_pep <- fread(paste0(TMP_DIR, "neo_pep.csv")) 

cohorts <- cohorts_ready %>% se(sampleId, cohort, acronym) %>% fi(sampleId %in% unique(neo_pep$sampleId))
cohorts_base <- rbind(cohorts, cohorts %>% mu(cohort = "Pan-Cancer")) 
cohort_summary <- cohorts_base %>% gb(cohort) %>% su(cohort_samples = n()) %>% ug() %>% mu(cohort_group = ifelse(cohort_samples < 20, "Other", cohort))
cohort_summary_group <- cohort_summary %>% gb(cohort_group) %>% su(samples = sum(cohort_samples)) %>% ug()
cohorts_go <- cohorts_base %>% lj(cohort_summary, by = "cohort")

fwrite(cohorts_go, paste0(TMP_DIR, "cohorts.csv"))
fwrite(cohort_summary, paste0(TMP_DIR, "cohorts_summary.csv"))
fwrite(cohort_summary_group, paste0(TMP_DIR, "cohorts_summary_group.csv"))
